...
const N8nWorkflow = () => {
  ...
  React.useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 1250) {
        // Calculate scale purely based on width, with a bit of padding
        const newScale = (window.innerWidth - 40) / 1250;
        setScale(newScale);
      } else {
        setScale(1);
      }
  ...
  return (
    <div style={{ 
      ...
      alignItems: 'center',
      minHeight: window.innerWidth < 1250 ? `${450 * scale + 40}px` : '500px',
      overflow: 'hidden'
    }}>
      <div 
        ref={containerRef}
      ...
      >
        ...
        >
          ...
          {/* ---- NODOS ---- */}
          <motion.div variants={nodeVariants} style={{ position: 'absolute', top: '160px', left: '40px' }}>
