    ...
    <div className="app">
      ...
      <section style={{ position: 'relative', height: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        ...
        <div style={{ position: 'absolute', bottom: '40px', left: '50%', transform: 'translateX(-50%)', zIndex: 10 }}>
          ...
          >
            {t('axia.hero.btn', 'Descubrir Beneficios')} ↓
          </motion.button>
        </div>
      </section>
      ...
      <section id="axia-features" className="section-container" style={{ position: 'relative', zIndex: 10, scrollMarginTop: '80px' }}>
        <motion.div {...fadeInUp} style={{ textAlign: 'center', marginBottom: '40px' }}>
          <h2 style={{ fontSize: 'clamp(1.6rem, 5vw, 2.5rem)', marginBottom: '15px' }}>{t('axia.features.title1', 'Potencia tu Negocio con')} <span className="gradient-text">AXIA</span>, {t('axia.features.title2', 'nuestros Asistentes virtuales para tu negocio')}</h2>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="glass" style={{ padding: '30px 20px', borderRadius: '20px', textAlign: 'center' }} whileHover={{ y: -10, border: '1px solid var(--color-accent)' }}>
            ...
            <h3 style={{ fontSize: '1.3rem', marginBottom: '10px', color: 'white' }}>{t('axia.f1.title', 'Disponibilidad 24/7')}</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: '1.5', fontSize: '0.95rem' }}>{t('axia.f1.desc', 'Tus clientes recibirán atención ininterrumpida, sin importar la hora o el día. AXIA nunca duerme.')}</p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="glass" style={{ padding: '30px 20px', borderRadius: '20px', textAlign: 'center' }} whileHover={{ y: -10, border: '1px solid var(--color-primary)' }}>
            ...
            <h3 style={{ fontSize: '1.3rem', marginBottom: '10px', color: 'white' }}>{t('axia.f2.title', 'IA Avanzada')}</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: '1.5', fontSize: '0.95rem' }}>{t('axia.f2.desc', 'Utiliza algoritmos de vanguardia para entender y responder preguntas complejas ofreciendo soluciones precisas.')}</p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="glass" style={{ padding: '30px 20px', borderRadius: '20px', textAlign: 'center' }} whileHover={{ y: -10, border: '1px solid var(--color-secondary)' }}>
            ...
            <h3 style={{ fontSize: '1.3rem', marginBottom: '10px', color: 'white' }}>{t('axia.f3.title', 'Procesamiento del Lenguaje')}</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: '1.5', fontSize: '0.95rem' }}>{t('axia.f3.desc', 'Interactúa de manera conversacional, comprendiendo el contexto y las intenciones del usuario de forma natural.')}</p>
          </motion.div>

          {/* Feature 4 */}
          <motion.div variants={itemVariants} className="glass" style={{ padding: '30px 20px', borderRadius: '20px', textAlign: 'center' }} whileHover={{ y: -10, border: '1px solid var(--color-accent)' }}>
            <Globe size={40} style={{ color: 'var(--color-accent)', margin: '0 auto 15px' }} />
            <h3 style={{ fontSize: '1.3rem', marginBottom: '10px', color: 'white' }}>{t('axia.f4.title', 'Integración Omnicanal')}</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: '1.5', fontSize: '0.95rem' }}>{t('axia.f4.desc', 'Integra AXIA sin esfuerzo en tu sitio web, WhatsApp, redes sociales y CRMs para una cobertura total.')}</p>
          </motion.div>

          {/* Feature 5 */}
          <motion.div variants={itemVariants} className="glass" style={{ padding: '30px 20px', borderRadius: '20px', textAlign: 'center' }} whileHover={{ y: -10, border: '1px solid var(--color-primary)' }}>
            <Zap size={40} style={{ color: 'var(--color-primary)', margin: '0 auto 15px' }} />
            <h3 style={{ fontSize: '1.3rem', marginBottom: '10px', color: 'white' }}>{t('axia.f5.title', 'Eficiencia Operativa')}</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: '1.5', fontSize: '0.95rem' }}>{t('axia.f5.desc', 'Reduce la carga de trabajo de tu equipo, automatizando tareas repetitivas y escalando tu soporte al instante.')}</p>
          </motion.div>
        </motion.div>
      </section>
      ...
      <section className="section-container" style={{ position: 'relative', zIndex: 10, paddingBottom: '60px', scrollMarginTop: '80px' }}>
        <motion.div {...fadeInUp} style={{ textAlign: 'center', marginBottom: '40px' }}>
          <h2 style={{ fontSize: 'clamp(1.4rem, 4vw, 2.2rem)', marginBottom: '15px' }}>{t('axia.workflow.title', '¿Cómo funciona nuestro')} <span className="gradient-text">{t('axia.workflow.titleHighlight', 'Agente IA?')}</span></h2>
          ...
          <p style={{ color: 'var(--color-text-muted)', maxWidth: '700px', margin: '20px auto 0', lineHeight: '1.6', fontSize: '0.95rem', padding: '0 15px' }}>
            {t('axia.workflow.desc', 'Observa en tiempo real cómo AXIA procesa la información, toma decisiones inteligentes y ejecuta acciones a través de tus herramientas empresariales favoritas.')}
          </p>
        </motion.div>
      ...
      <section className="section-container" style={{ position: 'relative', zIndex: 10, paddingBottom: '80px', scrollMarginTop: '80px' }}>
        <motion.div {...fadeInUp} style={{ textAlign: 'center', marginBottom: '50px' }}>
          <h2 style={{ fontSize: 'clamp(1.4rem, 4vw, 2.2rem)', marginBottom: '15px' }}>{t('axia.industries.title', 'Tu aliado en')} <span className="gradient-text">{t('axia.industries.titleHighlight', 'múltiples industrias')}</span></h2>
          <div style={{ width: '60px', height: '4px', background: 'var(--color-primary)', margin: '0 auto' }}></div>
          <p style={{ color: 'var(--color-text-muted)', maxWidth: '700px', margin: '20px auto 0', lineHeight: '1.6', fontSize: '0.95rem', padding: '0 15px' }}>
            {t('axia.industries.desc', 'Nuestros agentes de IA están diseñados para adaptarse a las necesidades específicas de tu sector, optimizando procesos y elevando la experiencia de tus usuarios.')}
          </p>
        </motion.div>
      ...
      <section className="section-container" style={{ textAlign: 'center', paddingBottom: '100px', position: 'relative', zIndex: 10 }}>
        <motion.div {...fadeInUp} className="glass" style={{ padding: 'clamp(30px, 5vw, 60px) 20px', borderRadius: '30px', maxWidth: '800px', margin: '0 auto', background: 'linear-gradient(135deg, rgba(0,86,179,0.1), rgba(0,180,255,0.05)) border-box' }}>
          <h3 style={{ fontSize: 'clamp(1.6rem, 5vw, 2.2rem)', marginBottom: '20px', color: 'white' }}>{t('axia.cta.title', 'Revoluciona tu')} <span className="gradient-text">{t('axia.cta.titleHighlight', 'Atención al Cliente')}</span></h3>
          <p style={{ color: 'var(--color-text-muted)', fontSize: '1rem', marginBottom: '30px' }}>{t('axia.cta.desc', 'Contacta a nuestro equipo de expertos para una demostración personalizada.')}</p>
          <motion.button
          ...
          >
            {t('axia.cta.btn', 'Solicitar Demostración')}
          </motion.button>
        </motion.div>
      </section>
