...
export default function NovaCore({ onNavigate }) {
  ...
  const [particleShape, setParticleShape] = React.useState('text');
  ...
  // Observers para detectar qué sección está en el viewport principal
  React.useEffect(() => {
    ...
    const handleIntersect = (entries) => {
      // Si el observer dispara por el montaje inicial, lo ignoramos para mantener 'text'
      ...
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          if (entry.target.id === 'hero-section') {
            ...
            setParticleShape('card');
          } else if (entry.target.id === 'info-section' || entry.target.id === 'nova-features' || entry.target.id === 'nova-cash-info') {
            // Cualquier sección de texto en medio dispersará las partículas
            setParticleShape('scatter');
          }
        }
      });
    };
  ...
  return (
    <div className="app">
      <NovaParticles targetShape={particleShape} />
